...
\paragraph{res\_entry\_start:}\\
    The first calendar day on which the person holds the position described by this resume entry. For parliamentary membership, this is the first day of the legal mandate to serve, distinguished from the date of swearing-in or the first sitting. The country-specific interpretation must be documented in the extraction pipeline. Dates use a two-digit day, three-letter lowercase English month and four-digit year; omit unknown day or month components. For Canada (House of Commons), PCC retains the source-recorded ParlInfo service starts under the explicit operational convention described under \texttt{leg\_period\_start}.
...
\paragraph{\texttt{leg\_period\_start}:}\\
    The first calendar day on which members of this parliament hold a legal mandate to serve, following the same mandate-based principle as \texttt{res\_entry\_start}. The date is determined by constitutional or statutory law, rather than when Parliament first convened or members were sworn in. The country-specific interpretation must be documented in the extraction pipeline. Use the PCC date format, omitting unknown day or month components.

    \textbf{Canada (House of Commons), operational convention adopted 08sep2026:} Use the corresponding main intake date already recorded in Canadian RESE and retain existing \texttt{res\_entry\_start} values. Record the first-session date and the reason for the earlier legislative start in \texttt{PARL.comment}. This source-based convention aligns PARL with recorded membership; it does not independently establish each member's precise declaration, writ-return, certification or oath date. For 1867 and 1872, use 20sep1867 and 12oct1872, the final polling dates corresponding to existing RESE intake. Preserve individual delayed entries, membership dates, \texttt{pers\_id} and \texttt{res\_entry\_id}. Derive \texttt{parliament\_id} from the corrected \texttt{leg\_period\_start} year; retain a month suffix only when multiple terms start in that year. For example, \texttt{CA\_NT-HC\_1926jan} becomes \texttt{CA\_NT-HC\_1925} (start 29oct1925), and \texttt{CA\_NT-HC\_1926dec} becomes \texttt{CA\_NT-HC\_1926} (start 14sep1926). Apply an explicit old-to-new mapping to the PARL keys together with all linked references and affected composite identifiers, including previous/next links and multi-value parliament references. Retain the first-session comments. The decision does not prescribe changes to legislative end dates.

...
**What counts as a mandate.** Holding a mandate is more than being able to cast a vote in a parliament that happens to be sitting. The test PCC applies is counterfactual: *if, for some urgent reason, parliament had to vote today (a war, a motion of no confidence, an emergency bill), who would be entitled to cast that vote?* Everyone for whom the answer is "this person" holds the mandate on that day, regardless of whether parliament is in session, whether the oath has already been taken, or whether such a vote ever takes place. Applied to an election: if, on the day after the poll, an emergency vote would be taken by the newly elected members (because the old parliament's authority has ended and the new members would be summoned and sworn in on the spot), then the new members' mandate, and thus their `res_entry_start`, begins right after the election, not at the first sitting or the oath. If the outgoing members would still be the ones voting until a constitutional or statutory hand-over (for example the fixed term start in the US), their mandate runs until that hand-over and the new members' mandate begins there. Swearing-in and first sittings are procedural consequences of the mandate and never define it; the first sitting is recorded separately in PARL `day_of_first_session`.
...
| 11 | Seated non-voting / observer member: occupies a seat and may vote in committees and internal parliamentary elections, but not on legislation (e.g. the West Berlin Bundestag deputies, "Berliner Abgeordnete", 1949–1990). Retained in RESE but excluded from `parliament_size` and the default parliamentary totals and representation statistics, which count voting members only |
...
| `day_of_first_session` | Date | 03jan2025, 07dec1863, 06nov1867 | **day of first session**: the day the newly elected parliament first convened (opening of its first session); auxiliary to the mandate-based `leg_period_start`, never earlier than it | CWA |
...
| `parliament_size` | Integer | 150, 435, `497;519;663` | **parliament size**: the number of voting seats in the parliament. If voting capacity changes within a term, the successive values are separated by `;` in chronological order | CWA |
| `comment` | String | "voting capacity rises from 487 to 497 on 04jan1957, when ten Saarland members join" | **free-text note**: optional human-readable annotation about the parliament row; typically empty. Used to document date conventions and first-session dates, or the reason and changeover date(s) behind a `;`-separated `parliament_size` | CWA |
...
#### leg_period_start
**Legislative period start date**: The first calendar day on which members of this parliament hold a legal mandate to serve. This follows the same mandate-based principle as `res_entry_start` in RESE: the date is determined by constitutional or statutory law, not by when parliament first convened or when members were sworn in. The exact legal basis varies by country and must be documented in the data extraction pipeline (see the definition of the mandate, including the "who would vote today" test, under `res_entry_start` in the RESE section). Date format: two-digit day, month (first three letters), and four-digit year (e.g., `03jan2025`). Shortened versions are used when day or month are unknown.

**Canada (House of Commons) - operational convention adopted 08sep2026:** `leg_period_start` uses the corresponding main intake date already recorded in Canadian RESE, while existing `res_entry_start` values are retained. The first-session date is recorded in `day_of_first_session`; `PARL.comment` is empty except for the two staggered elections, where it notes that ParlInfo's GeneralElectionDate is the final polling date. This is a documented source-based convention, not independent verification of each member's precise legal declaration, writ-return, certification or oath date. For the staggered elections of 1867 and 1872, the accepted main intake dates are `20sep1867` and `12oct1872`, corresponding to their final polling dates. Individual delayed entries, membership dates, `pers_id` and `res_entry_id` are preserved. Derive `parliament_id` from the corrected `leg_period_start` year; retain a month suffix only when multiple terms start in that year. For example, `CA_NT-HC_1926jan` becomes `CA_NT-HC_1925` (start `29oct1925`), and `CA_NT-HC_1926dec` becomes `CA_NT-HC_1926` (start `14sep1926`). Apply an explicit old-to-new mapping to the PARL keys together with all linked references and affected composite identifiers, including previous/next links and multi-value parliament references. Retain the staggered-election comments. This decision does not prescribe changes to legislative end dates.
...
#### day_of_first_session
**Day of first session**: The calendar day on which the newly elected parliament first convened in plenary, i.e. the opening day of its first (constituent) session. Same date format as `leg_period_start`. This is a descriptive, auxiliary date, not a mandate boundary: `leg_period_start` and `res_entry_start` follow the legal mandate (see the mandate test under `res_entry_start` in the RESE section), which may begin before the first sitting. `day_of_first_session` is therefore never earlier than `leg_period_start`. It equals it where the mandate legally begins with the constituent sitting (CH, DE, NL) and is later where the mandate begins earlier by law or convention (US: constitutional term start, 4 March until 1933 and 3 January from 1935; CA: election-day intake, see the Canada note under `leg_period_start`). Its purpose is diagnostic: comparing source membership starts with this date shows whether a source records mandate starts, sitting starts or oath dates, and it anchors election-induced turnover in daily seat counts. Sources are the official session calendars, documented in each extraction pipeline (US: House History "List All Sessions", used for both chambers; CA: ParlInfo parliament list `FirstSession`; CH: constituent session of the legislature). Missing values: a cell whose date has not been collected is simply left empty (as for most historical Swiss rows); `NA` means the date is not available. Never derive RESE dates from it.
...
#### parliament_size
**Parliament size — voting-only convention approved 16 September 2026, for all countries**: The number of voting seats in the parliament under the institutional composition in force. Ordinary vacancies do not reduce this capacity. Compare it with the number of active voting members, whose mandates use the RESE midnight rule; an occupancy count can therefore be lower than `parliament_size`. Members without a full legislative vote remain in RESE with `pf_position = 11` (`political_function` ending `_11`), but their seats are excluded here and they are excluded from default parliamentary coverage checks, membership totals and women's percentages. Seated substitutes (`09`) remain included while exercising a voting mandate. A change from non-voting to voting status must be dated in RESE and reflected in capacity where applicable. This supersedes the earlier all-seated convention, including the West Berlin example.

**Fluctuating sizes**: when the number of seats changes *within* a single term, the successive values are written as integers separated by `;` in chronological order — the same convention used for committee `seats` (see the COMM section). Each value is the size in force until the next changeover. For example, the 11th German Bundestag (`DE_NT-BT_1987`, 18feb1987–19dec1990) uses `497;519;663`: 497 voting seats through 20jun1990, 519 from the approved West Berlin voting-status boundary on 21jun1990, and 663 from 03oct1990 when 144 former GDR *Volkskammer* delegates joined. The 8 June Allied letter remains a preceding legal event; the coding follows the Bundestag's explicit 21 June status date. The intra-term changeover date itself is not stored in the `parliament_size` cell; consumers that need a daily size series supply it separately, and the `comment` field (see below) is where that changeover date and its reason are recorded in prose. A single-value size (the common case) is just one integer.

#### comment
**Free-text note**: An optional human-readable annotation about the parliament row. Empty for the vast majority of rows. Its main use so far is to document the reason and the changeover date(s) behind a `;`-separated `parliament_size`, since those dates are not stored in the `parliament_size` cell itself. For example, `DE_NT-BT_1953` records the rise from 487 to 497 voting seats when ten Saarland members joined on 04jan1957. Growth of a non-voting delegation alone, such as West Berlin in 1952, does not create a voting-capacity step. The field is descriptive only and is not parsed by any pipeline.
